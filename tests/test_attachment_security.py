...
def _no_clone(monkeypatch):
    monkeypatch.setattr(
        atool.git_tool,
        "clone",
        lambda url, dest: Path(dest).mkdir(parents=True, exist_ok=True),
    )
...
def test_sanitized_origin_strips_userinfo_query_and_scp():
    ...
    assert (
        "secret"
        not in _sanitized_git_origin("https://github.com/o/r?token=SECRET").lower()
    )
    assert (
        "secret"
        not in _sanitized_git_origin("https://github.com/o/r.git#SECRET").lower()
    )
...
def test_materialize_git_summary_has_no_userinfo_token(tmp_path, monkeypatch):
    ...
    records = atool.materialize(
        [{"kind": "git", "name": "repo", "uri": url}], str(tmp_path)
    )
