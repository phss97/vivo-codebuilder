...
4b. `build` cost cap — when `CODEBUILDER_MAX_RUN_COST_USD` is set, `run_executor` gets `budget_usd` (cap minus build-so-far). On `CCBudgetExceeded`, `build` writes `CHANGELOG.md` into `build_dir` (`CODEBUILDER_BUDGET_CHANGELOG` = `agent` bounded wrap-up call | `deterministic` git-diff+plan | `off`; `agent` falls back to deterministic) and marks the job failed-with-a-note. The partial package is delivered by `finalize`.
5. `finalize` (`@listen(build)`), **`async`** — on a **healthy** build: light deterministic QA (`_run_final_qa`), one optional executor repair pass on failure (`CODEBUILDER_MAX_FINAL_QA_REPAIRS`, default 1; repair also respects the cost budget). On a **failed** build (crash or budget stop) it skips QA but **still zips + uploads the partial `build_dir`** (incl. `CHANGELOG.md`) — only a "no plan / no build_dir" job returns empty. Then git diff (patch), zip, S3 upload, history, and the completion payload with `qa_passed` as the safety signal. Failed payloads may still carry `project_archive`/`zip_url`/`artifact_urls` — callers must gate on `qa_passed`. QA can skip the (free but slow) pytest sweep via `CODEBUILDER_RUN_TESTS=false` (ruff + uv-install still gate).
