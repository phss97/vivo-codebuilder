...
- `run_reviewer(cwd, prompt, ...)` — RPA-only, read-only (`Read/Grep/Glob/Skill`) and structured as `ProductionReview`. It runs only after deterministic QA passes and reports execution blockers in production wiring; non-RPA jobs skip it.
...
**Cost visibility + cap.** Every planner/executor/reviewer call reports `total_cost_usd`/tokens/turns via an `on_usage` callback → logged and emitted as an `llm_usage` progress event (on success *and* failure, so wasted spend is visible). Executor and reviewer calls share the build budget and raise `CCBudgetExceeded` once their streamed estimate crosses the remaining cap. Estimate rates: `CODEBUILDER_COST_PER_MTOK_INPUT`/`_OUTPUT`. It's a safety valve, not billing (may overshoot by a turn).
...
5. `finalize` (`@listen(build)`), **`async`** — always runs deterministic package QA when `build_dir` exists, including after an executor crash or budget stop. Once deterministic QA passes, an RPA build gets the read-only production-wiring review. Deterministic or semantic blockers share one executor repair allowance (`CODEBUILDER_MAX_FINAL_QA_REPAIRS`, default 1); failed builders and reviewer infrastructure failures never trigger repair. It then captures the patch, zips, uploads, records history, and emits the completion payload. Any failed archive gets a deterministic `CODEBUILDER_REPORT.md` injected by `_zip_build` without touching the source tree. The report contains changed files, preflight/final results, plan, repair count, and next work. Successful archives omit it. Failed payloads remain `status=failed`, `qa_passed=false` while retaining the archive/artifacts/patch when available.
